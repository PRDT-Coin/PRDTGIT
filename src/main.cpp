...
uint256 hashGenesisBlock("0x2b42fae95050ec36ecc2d0f9a6f71fc6f3e08a22c446bcfb16edb2fff33b3a98");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PREDATORCOIN: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PREDATORCOIN Signed Message:\n";
...
{
    // PREDATORCOIN: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 8250 * COIN;

    nSubsidy >>= (nHeight / 62100);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PREDATORCOIN: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PREDATORCOIN: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PREDATORCOIN: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2b42fae95050ec36ecc2d0f9a6f71fc6f3e08a22c446bcfb16edb2fff33b3a98");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1413407733;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084932111;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PREDATORCOINMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PREDATORCOINMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running PREDATORCOIN with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PREDATORCOINMiner terminated\n");
        throw;
    }
}
